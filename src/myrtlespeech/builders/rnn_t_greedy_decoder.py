...
def build(
    ...
    Args:
        rnn_t_greedy_decoder_cfg: A ``RNNTGreedyDecoder`` protobuf object containing
            the config for the desired :py:class:`RNNTGreedyDecoder`.
    ...
    if (
        ...
        if max_symbols_per_step < 1:
            raise ValueError(
                f"rnn_t_greedy_decoder_cfg.max_symbols_per_step = {max_symbols_per_step} but this must be >= 1"
            )
    else:

...
def test_build_raises_value_error_when_model_invalid(
    stt_cfg: speech_to_text_pb2.SpeechToText,
) -> None:
    """Test that build raises error for invalid model + loss."""
    loss_type = stt_cfg.WhichOneof("supported_losses")
    model_type = stt_cfg.WhichOneof("supported_models")
    if loss_type == "ctc_loss":
        assume("transducer" in model_type)
    elif loss_type == "transducer_loss":
        assume("transducer" not in model_type)
    else:
...
@given(stt_cfg=speech_to_texts(valid_only=False))
def test_build_raises_value_error_when_post_process_invalid(
    stt_cfg: speech_to_text_pb2.SpeechToText,
) -> None:
    """Test that build raises error for invalid model + loss."""
    loss_type = stt_cfg.WhichOneof("supported_losses")
    ...
    if loss_type == "ctc_loss":
        assume("transducer" in post_process_type)
    elif loss_type == "transducer_loss":
        assume("transducer" not in post_process_type)
    else:
        raise ValueError(f"loss_type={loss_type} not recognized")
